				...
				if (landplot.minx==x) {  // landplot is adjacent to western vertical wall
					if (corner0.y>corner1.y) {   // wall goes from north to south
						if (corner0.y>landplot.maxy)
							newPoints.push({x:x,y:landplot.maxy});
						newPoints.push({x:landplot.maxx,y:landplot.maxy});
						newPoints.push({x:landplot.maxx,y:landplot.miny});
						if (landplot.miny>corner1.y) 
							newPoints.push({x:x,y:landplot.miny});
					...
					if (corner0.y>corner1.y) {   // wall goes from north to south
						if (corner0.y>landplot.maxy)
							newPoints.push({x:x,y:landplot.maxy});
						newPoints.push({x:landplot.minx,y:landplot.maxy});
						newPoints.push({x:landplot.minx,y:landplot.miny});
						if (landplot.miny>corner1.y) 
							newPoints.push({x:x,y:landplot.miny});
					} else if (corner0.y<corner1.y) { // wall goes from south to north
						if (corner0.y<landplot.miny)
				...
				if (landplot.miny==y) {  // landplot is adjacent to southern horizontal wall
					if (corner0.x>corner1.x) {   // wall goes from east to west
						if (corner0.x>landplot.maxx)
							newPoints.push({y:y,x:landplot.maxx});
						newPoints.push({y:landplot.maxy,x:landplot.maxx});
						newPoints.push({y:landplot.maxy,x:landplot.minx});
						if (landplot.minx>corner1.x) 
							newPoints.push({y:y,x:landplot.minx});
					...
					if (corner0.x>corner1.x) {   // wall goes from east to west
						if (corner0.x>landplot.maxx)
							newPoints.push({y:y,x:landplot.maxx});
						newPoints.push({y:landplot.miny,x:landplot.maxx});
						newPoints.push({y:landplot.miny,x:landplot.minx});
						if (landplot.minx>corner1.x) 
							newPoints.push({y:y,x:landplot.minx});
					} else if (corner0.x<corner1.x) { // wall goes from west to east
						if (corner0.x<landplot.minx)
