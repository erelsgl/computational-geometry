...
describe('square-covering structures', function() {
	...
	it('removes erasable regions in rectangles', function () {
		...
		srp.getResidualPolygon().getCoordinates2D().should.eql(
				[ { x:  0, y: 25 },
				  { x: 10, y: 25 },
				  { x: 10, y: 35 },
				  { x:  0, y: 35 },
		...
		srp.corners.pluck("x").should.eql([ 0,10,10, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0,25,25]);
		srp.removeErasableRegion(srp.segments.first.next.next);
		srp.corners.pluck("x").should.eql([ 0,10,10, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0,15,15]);
		srp.removeErasableRegion(srp.segments.first.next.next);
		srp.corners.pluck("x").should.eql([ 0,10,10, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0,10,10]);
		...
		srp.corners.pluck("y").should.eql([ 5, 5,20,20]);
//		console.log("0: "+srp)
		srp.removeErasableRegion(srp.segments.first.next);
//		console.log("1: "+srp)
		srp.corners.pluck("x").should.eql([ 0,15,15, 0]);
		srp.corners.pluck("y").should.eql([ 5, 5,20,20]);
	...
	it('removes erasable regions in L-shapes', function () {
		...
		srp.corners.pluck("x").should.eql([10,20,20, 0, 0,10]);
		srp.corners.pluck("y").should.eql([ 0, 0,20,20,10,10]);
		srp.removeErasableRegion(srp.segments.first);
		srp.corners.pluck("x").should.eql([20,20,0,0]);
	...
	it('removes erasable regions in special L-shapes', function() {
		var srp = new jsts.algorithm.MinSquareCoveringData(new jsts.geom.SimpleRectilinearPolygon([0,0, 20,1, 21,30], factory));
		srp.corners.pluck("x").should.eql([ 0,20,20,21,21, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0, 1, 1,30,30]);
		srp.removeErasableRegion(srp.segments.first.next.next.next.next);
		srp.corners.pluck("x").should.eql([ 0,20,20,21,21, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0, 1, 1,22,22]);
	})

	it.skip('removes erasable regions in special shapes', function() {
		var srp = new jsts.algorithm.MinSquareCoveringData(new jsts.geom.SimpleRectilinearPolygon([0,1, 1,0.23749999999999993, 0.7625000000000001,0.23750000000000004, 0.525,0, 0.2325,0.2325], factory));
		srp.removeErasableRegion(srp.segments.first.next.next);
	})
	...
	it('removes erasable regions in T-shapes', function () {
		...
		srp.corners.pluck("x").should.eql([ 0,25,25,35,35,25,25, 0]);
		srp.corners.pluck("y").should.eql([10,10, 0, 0,30,30,20,20]);
		...
		srp.corners.pluck("y").should.eql([ 0, 0,30,30]);
	});
	...
	it('removes erasable regions with 3-knob continuators', function () {
		...
		srp.corners.pluck("x").should.eql([ 0,40,40,30,30,40,40, 0, 0,10,10, 0]);
		srp.corners.pluck("y").should.eql([ 0, 0,40,40,50,50,90,90,50,50,40,40]);
		srp.segments.last.knobCount=3;
		srp.removeErasableRegion(srp.segments.last);
		srp.corners.pluck("x").should.eql([10,30,30,40,40, 0, 0,10]);
