...
jsts.algorithm.halfProportionalDivision4Walls = function(agentsValuePoints, envelope, maxAspectRatio) {
	...
	var landplots = runDivisionAlgorithm(
			normalized4WallsAlgorithm, shorterSide /* The normalized4WallsAlgorithm algorithm assumes that the southern side is shorter */,
...
jsts.algorithm.halfProportionalDivision3Walls = function(agentsValuePoints, envelope, maxAspectRatio, openSide) {
	...
	var landplots = runDivisionAlgorithm(
			normalized3WallsAlgorithm, southernSide,
			valueFunctions, envelope, maxAspectRatio);
	landplots.forEach(roundFields3);
	return landplots;
};
...
var runDivisionAlgorithm = function(normalizedDivisionFunction, southernSide, valueFunctions, envelope, maxAspectRatio) {
	if (envelope.minx==undefined || envelope.maxx==undefined || envelope.miny==undefined || envelope.maxy==undefined)
		throw new Error("Illegal envelope: "+JSON.stringify(envelope))
	...
	enveloper = jsts.algorithm.transformAxisParallelRectangle(rotateTransformation,
		{minx:envelope.minx, maxx:envelope.maxx, miny:envelope.miny, maxy:envelope.maxy});
	...
	var transformedvalueFunctions = valueFunctions.map(function(valueFunction) {
		...
		var valuePointsInEnvelope = jsts.algorithm.pointsInEnvelope(valueFunction.points, envelope);
		if (!valuePointsInEnvelope.length)
			throw new Error("No valuePointsInEnvelope! envelope="+JSON.stringify(envelope)+" valueFunction="+JSON.stringify(valueFunction));
		var transformedValuePoints = valuePointsInEnvelope.map(jsts.algorithm.transformedPoint.bind(0,transformation));
		return valueFunction.cloneWithNewPoints(transformedValuePoints);
	});
 ...
 */
var normalized4WallsAlgorithm = function(valueFunctions, yLength, maxAspectRatio) {
	...
	console.dir(_.pluck(valueFunctions,"points"))
	...
	if (false) {
		...
		for (var k=1; k<=numOfAgents-1; ++k) {
			...
			if (y_2k <= yLength-0.5) {  // North will be 2-fat; South may be 2-thin.
				...
				var southPlots = runDivisionAlgorithm(normalized4WallsAlgorithm, /*shorter side = */(y>1? jsts.Side.South: jsts.Side.East),            southAgents, south, maxAspectRatio);
				if (southPlots.length==southAgents.length) {
					...
					var northPlots = runDivisionAlgorithm(normalized4WallsAlgorithm, /*shorter side = */(yLength-y>1? jsts.Side.South: jsts.Side.East),    northAgents, north, maxAspectRatio);
	...
	if (TRY_OPEN_PARTITIONS) {
		TRACE(numOfAgents, "## Trying open partitions");
		for (var k=1; k<=numOfAgents-1; ++k) {
			var y_2k = yCuts_2k[k];           // the k-th 2k line
			if (y_2k==undefined)
				throw new Error("y_2k is undefined: yCuts_2k="+JSON.stringify(yCuts_2k))
			var y_2k_next = yCuts_2k_next[k]; // the k+1-th 2k line
			orderValueFunctionsByYcut(valueFunctions, 2*k);
			if (y_2k <= yLength-0.5) {  // South is 2-thin (y_2k<y_2k_next<0.5)
				...
				var southPlots = runDivisionAlgorithm(normalized3WallsAlgorithmThin, /*open side=north, previous side= */jsts.Side.East, southAgents, south, maxAspectRatio);
				...
				var northPlots = runDivisionAlgorithm(normalized4WallsAlgorithm, /*shorter side = */(yLength-highestSouthPlot.maxy>1? jsts.Side.South: jsts.Side.East),    northAgents, north, maxAspectRatio);
				landplots = northPlots.concat(southPlots);
				if (landplots.length==numOfAgents) {
				...
				var north = {minx:0, maxx:1, miny:y, maxy:yLength};
				var northAgents = valueFunctions.slice(k-1, numOfAgents);
				var northPlots = runDivisionAlgorithm(normalized3WallsAlgorithmThin, /*open side=south, previous side = */jsts.Side.West, northAgents, /*envelope=*/north, maxAspectRatio);
				...
				var southAgents = valueFunctions.slice(0, k-1);
				var southPlots = runDivisionAlgorithm(normalized4WallsAlgorithm, /*shorter side = */(lowestNorthPlot.miny>1? jsts.Side.South: jsts.Side.East),    southAgents, south, maxAspectRatio);
				landplots = northPlots.concat(southPlots);
				if (landplots.length==numOfAgents) {
 ...
 */
var normalized3WallsAlgorithm = function(valueFunctions, yLength, maxAspectRatio) {
	var numOfAgents = valueFunctions.length;
	...
	var south = {minx:0, maxx:1, miny:0, maxy:y};
	var southPlots = runDivisionAlgorithm(normalized4WallsAlgorithm, (y>1? jsts.Side.South: jsts.Side.East),   southAgents, south, maxAspectRatio);
	if (southPlots.length==southAgents.length) {
	...
	var plots = runDivisionAlgorithm(normalized3WallsAlgorithmThin, /*open side=north, previous side=*/ jsts.Side.East,   valueFunctions, south, maxAspectRatio);
 ...
 */
var normalized3WallsAlgorithmThin = function(valueFunctions, yLength, maxAspectRatio) {
	var numOfAgents = valueFunctions.length;
	...
	for (var k=1; k<=numOfAgents-1; ++k) {
		...
		if (1 <= y_2k_1_next && y_2k_1 <= yLength-1) {  // both North and South are 2-fat
			...
			var southPlots = runDivisionAlgorithm(normalized3WallsAlgorithm, jsts.Side.West,   southAgents, south, maxAspectRatio),
			    northPlots = runDivisionAlgorithm(normalized3WallsAlgorithm, jsts.Side.West,    northAgents, north, maxAspectRatio);
			return southPlots.concat(northPlots);
		}
	}
